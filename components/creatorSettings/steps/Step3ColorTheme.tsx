...
export default function LearnHereSection({
  ...
  const handleImageUpload = async (file: File | null) => {
    ...
    try {
      setIsUploading(true);
      onUploadStart?.();
      const response = await uploadImage(file, "section3");
      onChange({
        sections: {
          ...formData.sections,
          section3: {
            ...formData.sections.section3,
            image: response.url,
          },
        },
      });
      ...
      <div className="flex flex-col gap-15 bg-gray-50 md:border border-[#C3C3C3] md:bg-transparent px-4 py-6 rounded-2xl">
        <div>
          ...
          <textarea
            ...
            value={formData.sections.section3.description || ""}
            onChange={(e) =>
              onChange({
                sections: {
                  ...formData.sections,
        ...
        <div>
          ...
          <label className="cursor-pointer block">
            <div className="w-full h-48 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition-colors relative">
              ...
              ) : formData.sections.section3.image ? (
                <Image
                  src={formData.sections.section3.image}
                  width={800}
                  height={200}
                  alt="Why Learn Here"
