...
export default function WhatYouWillLearn({
  ...
  const handleImageUpload = async (file: File | null) => {
    ...
    try {
      setIsUploading(true);
      onUploadStart?.();
      const response = await uploadImage(file, "section4");
      onChange({
        sections: {
          ...formData.sections,
          section4: {
            ...formData.sections.section4,
            image: response.url,
          },
        },
      });
      ...
      <div className="flex flex-col gap-15 bg-gray-50 md:border border-[#C3C3C3] md:bg-transparent px-4 py-6 rounded-2xl">
        <div>
          ...
          <input
            ...
            value={formData.sections.section4.title || ""}
            onChange={(e) =>
              onChange({
                sections: {
                  ...formData.sections,
        ...
        <div>
          ...
          <textarea
            ...
            value={formData.sections.section4.description || ""}
            onChange={(e) =>
              onChange({
                sections: {
                  ...formData.sections,
        ...
        <div>
          ...
          <label className="cursor-pointer block">
            <div className="w-full h-48 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition-colors relative">
              ...
              ) : formData.sections.section4.image ? (
                <Image
                  src={formData.sections.section4.image}
                  width={800}
                  height={200}
